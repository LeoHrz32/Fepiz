const {response} = require('express'); // Importa la función `response` 
// desde el módulo express 
const bcrypt = require('bcryptjs'); // Importa la librería bcryptjs para el 
// cifrado de contraseñas 
 
// Importar modelos 
const Usuario = require('../modules/usuario'); // Importa el modelo de 
// Usuario desde el módulo '../modules/usuario' 
...
const usuariosGet = async (req, res = response) => { 
    ...
    const {q, nombre, page= 1, limit} = req.query; // Extrae los parámetros 
// de la consulta 
 
    const usuarios = await Usuario.find(); // Consulta todos los documentos 
// de la colección Usuario 
 
    res.json({ 
        usuarios // Devuelve un objeto JSON con los usuarios obtenidos de la 
// base de datos 
    }); 
} 
...
// de la colección Usuario 
    ...
    usuarios.forEach(numero => console.log(numero)); // Muestra cada 
// documento de usuario por consola 
 
    res.json({ 
        msg: 'Prom API controlador', // Devuelve un mensaje indicando que es 
// el controlador del promedio 
        q, 
        nombre, 
        page, 
        limit, 
...
const usuariosPost = async(req, res = response) => { 
    ...
    const usuario = new Usuario(body); // Crea un nuevo objeto Usuario con 
// los datos del cuerpo de la solicitud 
 
    const {nombre, email, password, rol, estado} = req.body; // Extrae los 
// datos del cuerpo de la solicitud 
 
    try { 
        // Encripta la contraseña antes de guardarla en la base de datos 
        const salt = bcrypt.genSaltSync(10); // Genera una sal para el 
cifrado 
...
const usuariosDelete = async(req, res = response) => { 
    ...
    const {nombre, email, password, rol, estado} = req.query; // Extrae los 
// datos del cuerpo de la solicitud 
//  
...
// Exporta los controladores de las rutas de usuarios para que estén 
